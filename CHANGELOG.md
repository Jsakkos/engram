...
- **That same outage no longer fails the install part-way through, either.** Retrying covered the version lookup but not the much larger MakeMKV source download that follows it, which is the longer request against the same server and so the more exposed of the two. A brief network or Cloudflare hiccup while that download was in flight failed the install immediately. It is now retried, and if it does give up it reports whether the site was unreachable or the requested MakeMKV version is simply no longer published. (#559)
