...
class Addic7edClient:
    ...
    def get_episode_subtitles(
        ...
        logger.info(f"Found {len(subtitles)} {language} subtitles for {show_name} S{season:02d}E{episode:02d}")
        return subtitles
    
    def _parse_subtitle_table(self, soup: BeautifulSoup, language: str, referer: str) -> list[SubtitleEntry]:
    ...
    def _parse_single_subtitle(self, table: BeautifulSoup, language: str, referer: str) -> SubtitleEntry | None:
    ...
    def _extract_version_info(self, parent_element, link) -> str:
        ...
        text = parent_element.get_text()
        groups = ["LOL", "DIMENSION", "KILLERS", "FoV", "IMMERSE", "AVS", "2HD", 
                  "ASAP", "EXCELLENCE", "FLEET", "TLA", "PROPER", "REPACK"]
        ...
        for episode in range(1, episode_count + 1):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    logger.warning(f"Attempt {attempt + 1}/{max_retries} failed for {episode_code}: {e}")
