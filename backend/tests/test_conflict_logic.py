...
def simulate_conflict_resolution(titles: List[MockTitle]):
    ...
    for ep_code, title_list in candidates.items():
        ...
        print(f"  Winner: Title {best_title.id} (votes={best['vote_count']}, score={best['score']:.3f}, cov={best['file_coverage']:.1%})")
        ...
        if len(ranked_candidates) > 1:
            ...
            if (best["vote_count"] == runner_up["vote_count"] and
                abs(best["score"] - runner_up["score"]) < 0.05):
                print(f"  [WARNING] Ambiguous match! Same votes, score margin: {best['score'] - runner_up['score']:.3f}")

if __name__ == "__main__":
    # Test Case 1: Clear Winner (Individual vs Play All) - vote count wins
    titles_1 = [
        MockTitle(id=1, title_index=1, matched_episode="S01E01", match_confidence=0.9,
                  match_details=json.dumps({"score": 0.714, "vote_count": 10, "file_cov": 0.95})), # Individual episode
        MockTitle(id=2, title_index=0, matched_episode="S01E01", match_confidence=0.9,
                  match_details=json.dumps({"score": 0.737, "vote_count": 1, "file_cov": 0.017}))   # Play All (opening credits)
    ]
    simulate_conflict_resolution(titles_1)

    # Test Case 2: Ambiguous (Two similar files with same votes)
    titles_2 = [
        MockTitle(id=3, title_index=3, matched_episode="S01E02", match_confidence=0.8,
                  match_details=json.dumps({"score": 0.72, "vote_count": 5, "file_cov": 0.15})),
        MockTitle(id=4, title_index=4, matched_episode="S01E02", match_confidence=0.79,
    ...
    titles_3 = [
        MockTitle(id=5, title_index=5, matched_episode="S01E03", match_confidence=0.9, match_details=None), # Old format (no votes)
        MockTitle(id=6, title_index=0, matched_episode="S01E03", match_confidence=0.9,
                  match_details=json.dumps({"score": 0.15, "vote_count": 1, "file_cov": 0.02})), # Play All
    ]
    simulate_conflict_resolution(titles_3)

    # Test Case 4: Low-vote false positives (Arrested Development scenario)
    titles_4 = [
        MockTitle(id=7, title_index=52, matched_episode="S01E06", match_confidence=0.737,
                  match_details=json.dumps({"score": 0.737, "vote_count": 1, "file_cov": 0.017})),
        MockTitle(id=8, title_index=53, matched_episode="S01E06", match_confidence=0.737,
                  match_details=json.dumps({"score": 0.737, "vote_count": 1, "file_cov": 0.017})),
        MockTitle(id=9, title_index=54, matched_episode="S01E06", match_confidence=0.714,
                  match_details=json.dumps({"score": 0.714, "vote_count": 10, "file_cov": 0.152})), # Actual episode
    ]
