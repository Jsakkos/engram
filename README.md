...
Engram ships with the **Synapse v2** brand system — a cyberpunk / Blade Runner 2049 aesthetic with near-black surfaces, cyan + magenta accents, and surveillance-equipment chrome. The mark is three open arcs (disc rings / hippocampal replay) crossed by a horizontal "read-line" terminating in a node — a memory trace being written.

The canonical brand spec lives at [docs/design_handoff_brand/README.md](docs/design_handoff_brand/README.md). Developer guidance for using the brand primitives is in [docs/development/brand.md](docs/development/brand.md). Raster assets (favicons, `.ico`, `.icns`, Linux PNGs) are generated by `npm run brand:export` from the SVG sources in `frontend/public/brand/sources/`.
