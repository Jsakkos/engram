...
- **Duration pre-filter trust is per disc**: `MatchingCoordinator._runtime_filter_policy`
  ...
  that the runtime list is wrong. `check_job_completion` also routes an all-extras TV disc to
  review rather than filing the lot into `Extras/` and reporting success.
