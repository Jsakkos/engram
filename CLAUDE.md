...
- **Parallel-PR conflicts auto-resolve.** Every PR appends to `[Unreleased]`, so concurrent PRs used to collide there on every merge. `CHANGELOG.md merge=union` in `.gitattributes` (git's built-in union driver — no per-clone config) keeps **both** sides of a conflicting `[Unreleased]` hunk, so a post-merge `git rebase` combines the entries automatically with no hand-editing. Caveats are cosmetic and absorbed by the release-PR curation pass: bullet order is "upstream-then-yours," and two PRs introducing *different* new `### ` subsections keep both. It never affects release extraction (that only reads finished `## [X.Y.Z]` sections). Assessment: `docs/superpowers/reviews/2026-06-02-changelog-conflict-friction.md`.
