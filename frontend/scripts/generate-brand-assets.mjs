...
// favicon.svg — copy the full mark SVG (browsers that prefer SVG will use this)
...
const manifest = {
  ...
  favicons: FAVICON_SIZES.map(({ px }) => `favicons/favicon-${px}.png`).concat(
    ["favicons/favicon.svg", "favicons/favicon.ico"]
  ),
...
writeFileSync(join(BRAND_DIR, "manifest.json"), JSON.stringify(manifest, null, 2));
console.log(`\n✓ Wrote ${Object.values(manifest).flat().length - 1} brand assets to ${BRAND_DIR}`);
