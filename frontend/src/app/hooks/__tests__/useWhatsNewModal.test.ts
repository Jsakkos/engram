...
type HookProps = { status: UpdateStatus | null; setupComplete: boolean | null };

describe("useWhatsNewModal", () => {
    beforeEach(() => localStorage.clear());

    it("seeds silently on a fresh install", () => {
        const { result } = renderHook(() => useWhatsNewModal(status("0.28.0", "notes"), false));
        expect(result.current.open).toBe(false);
        expect(localStorage.getItem(KEY)).toBe("0.28.0");
    });

    it("opens for an existing install that predates the lastSeenVersion key", () => {
        const { result } = renderHook(() => useWhatsNewModal(status("0.28.0", "notes"), true));
        expect(result.current.open).toBe(true);
        expect(localStorage.getItem(KEY)).toBe("0.28.0");
    });

    it("does nothing, and does not seed, while the config has not loaded", () => {
        const { result } = renderHook(() => useWhatsNewModal(status("0.28.0", "notes"), null));
        expect(result.current.open).toBe(false);
        expect(localStorage.getItem(KEY)).toBeNull();
    });

    it("defers the decision until setupComplete resolves", () => {
        const initialProps: HookProps = { status: status("0.28.0", "notes"), setupComplete: null };
        const { result, rerender } = renderHook(
            (props: HookProps) => useWhatsNewModal(props.status, props.setupComplete),
            { initialProps },
        );
        expect(result.current.open).toBe(false);
        expect(localStorage.getItem(KEY)).toBeNull();

        rerender({ status: status("0.28.0", "notes"), setupComplete: true });
        expect(result.current.open).toBe(true);
        expect(localStorage.getItem(KEY)).toBe("0.28.0");
    });
    ...
    it("opens once when the version changed and notes are available", () => {
        ...
        const { result, rerender } = renderHook(() =>
            useWhatsNewModal(status("0.28.0", "notes"), true),
        );
        expect(result.current.open).toBe(true);
        expect(localStorage.getItem(KEY)).toBe("0.28.0");
    ...
    it("does not open, and does not advance the key, when notes are missing", () => {
        localStorage.setItem(KEY, "0.27.0");
        const { result } = renderHook(() => useWhatsNewModal(status("0.28.0", null), true));
        expect(result.current.open).toBe(false);
    ...
    it("does nothing when there is no status yet", () => {
        const { result } = renderHook(() => useWhatsNewModal(null, true));
        expect(result.current.open).toBe(false);
        expect(localStorage.getItem(KEY)).toBeNull();
    });
    ...
    it("shows notes that arrive later in the session without losing the version", () => {
        localStorage.setItem(KEY, "0.27.0");
        const initialProps: HookProps = { status: status("0.28.0", null), setupComplete: true };
        const { result, rerender } = renderHook(
            (props: HookProps) => useWhatsNewModal(props.status, props.setupComplete),
            { initialProps },
        );
